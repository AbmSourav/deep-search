...
const SearchOptions = ({ props, queryState }) => {
    ...
    const optionsStyles = {
        control: (baseStyles) => ({
            ...baseStyles,
            backgroundColor: adjustColor(attibutes?.optionsBackgroundColor) + '!important' || '#fff',
            borderColor: attibutes?.optionsBackgroundColor + '!important' || '#fff',
            boxShadow: attibutes?.optionsBackgroundColor ? `0 0 0 1px ${attibutes?.optionsBackgroundColor}` + '!important' : '0 0 0 1px #b4b4b4',
            color: attibutes?.optionsTextColor + '!important' || '#959595',
        }),
        option: (styles, {isFocused}) => ({
            ...styles,
            backgroundColor: isFocused ? adjustColor(attibutes?.optionsBackgroundColor) || '#83aeeb78' : (attibutes?.optionsBackgroundColor || '#fff'),
            color: attibutes?.optionsTextColor || 'initial',
            ':active': {
                ...styles[':active'],
                backgroundColor: adjustColor(attibutes?.optionsBackgroundColor) || 'initial'
            }
        }),
        multiValue: (styles) => {
            return {
    ...
    return (
        ...
        <style>{
            `
            .ds__placeholder {color: ${attibutes?.optionsTextColor || 'inherit'};}
            .ds__menu {background-color: ${attibutes?.optionsBackgroundColor || '#fff'};}
            .ds-options {border-top-color: ${adjustColor(attibutes?.backgroundColor) || '#e2e2e2'} !important;}
            `
        }</style>
