...
const PostList = ({ props, resData, queryState, rest }) => {
    ...
    return (
        <div className="ds-postlist">
            <style>{`
                .ds-postlist__post {border-bottom-color: ${
                    adjustColor(attibutes?.backgroundColor) || '#eee'
                } !important;}
                .ds-postlist__inner {scrollbar-color: #b4b4b4 ${
                    adjustColor(attibutes?.backgroundColor) || '#e3e3e37c'
                } !important;}
                `}</style>

            <div
                className="ds-postlist__header"
                style={{
                    backgroundColor:
                        adjustColor(attibutes?.backgroundColor) || '#eee',
                }}
            >
                <div
                    className="ds-postlist__header-close"
                    onClick={handleClose}
                >
                    <svg
                        style={{ fill: adjustColor(attibutes?.textColor) }}
                        ...
                        {queryRes.posts.map((post, index) => (
                            <div className="ds-postlist__post">
                                <a
                                    ...
                                    style={{
                                        color:
                                            attibutes?.postTitleColor ||
                                            '#027141',
                                        fontSize:
                                            attibutes?.postTitleFontSize ||
                                            '30px',
                                    }}
                                >
                                    {post.title}
                            ...
                            <div className="ds-postlist__pagination">
                                {queryRes?.prevPage > 0 && (
                                    <button
                                        ...
                                        onClick={() =>
                                            handlePagination(queryRes?.prevPage)
                                        }
                                ...
                                {queryRes?.nextPage > 0 && (
                                    <button
                                        className="ds-postlist__pagination-btn"
                                        onClick={() =>
                                            handlePagination(queryRes?.nextPage)
                                        }
                                        style={paginationStyles}
                                    >
                                        {__('Next', 'deep-search')}
